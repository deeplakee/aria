...
{
    auto self = as_obj_map(args[-1]);
    self->map_->insert(args[0], args[1]);
...
{
    auto self = as_obj_map(args[-1]);
    bool result = self->map_->has(args[0]);
...
{
    auto self = as_obj_map(args[-1]);
    return NanBox::fromNumber(self->map_->size());
}

static Value builtin_empty(AriaEnv *env, int argCount, Value *args)
{
    auto self = as_obj_map(args[-1]);
    return NanBox::fromBool(self->map_->empty());
}
...
{
    auto self = as_obj_map(args[-1]);
    ObjList *list = self->map_->create_key_list();
...
{
    auto self = as_obj_map(args[-1]);
    ObjList *newList = self->map_->create_value_list();
...
{
    auto self = as_obj_map(args[-1]);
    ObjList *list = self->map_->create_pair_list();
    return NanBox::fromObj(list);
}
...
static Value builtin_equals(AriaEnv *env, int argCount, Value *args)
{
    ...
    bool result = self->map_->equals(as_obj_map(args[0])->map_);
    return NanBox::fromBool(result);
}
